...
async function loadBuilderConfig(supabase) {
  ...
  return {
    template: data.template?.categories?.length ? data.template : fallbackTemplate,
    theme: data.theme && Object.keys(data.theme).length ? data.theme : fallbackTheme,
...
function assertExistingChannelsUntouched(before, guild) {
  for (const [id, previous] of before.entries()) {
    ...
    if ((current.parentId || null) !== previous.parentId) {
      throw new Error("SAFE_MODE_EXISTING_PARENT_CHANGED");
    }
    if (current.rawPosition !== previous.rawPosition) {
      throw new Error("SAFE_MODE_EXISTING_POSITION_CHANGED");
    }
  }
}
...
export async function inspectServerDiff(guild, template) {
...
export async function buildServer(guild, builderConfig) {
  const template = builderConfig.template || fallbackTemplate;
  const theme = builderConfig.theme || fallbackTheme;

  if (
    builderConfig.safe_mode !== true ||
    !localSafety.safeMode ||
    localSafety.behavior !== "append_only"
  ) {
...
async function queueBuilderJob(supabase, guildId) {
  const { data, error } = await supabase
...
export async function handleBuilderInteraction(
  ...
  if (
    ...
    interaction.commandName === "preview-tema"
  ) {
    if (!interaction.guild) {
      await interaction.reply({
        content: "❌ Use dentro de um servidor.",
        ephemeral: true,
      });
      return true;
    }

    const builderConfig = await loadBuilderConfig(supabase);
    const diff = await inspectServerDiff(
      interaction.guild,
  ...
  if (
    interaction.isChatInputCommand() &&
    interaction.commandName === "montar-servidor"
  ) {
    if (!interaction.guild) {
      await interaction.reply({
        content: "❌ Use dentro de um servidor.",
    ...
    await interaction.reply({
      ...
      content: diff.length
        ? "Serão criados **" +
          diff.length +
          "** item(ns). Nada existente será alterado."
        : "✅ Nada para criar.",
  ...
  if (
    ...
    !interaction.customId.startsWith("builder:")
  ) {
    return false;
  }
  ...
  if (action === "confirm") {
    try {
      const job = await queueBuilderJob(
        supabase,
      ...
      await interaction.update({
        content:
          "✅ Server Builder colocado na fila do Bot Core. Job: " +
          job.id +
          "\nAcompanhe pelo painel /admin/discord.",
        embeds: [],
        components: [],
      });
    } catch (error) {
      ...
      await interaction.update({
        content:
          "❌ Não foi possível colocar o Server Builder na fila.",
        embeds: [],
        components: [],
      });
    }
...
export function startBuilderWorker(client, supabase, config) {
  ...
  const tick = async () => {
    ...
    try {
      ...
      if (builderConfig.safe_mode !== true) {
        throw new Error("SAFE_MODE_REQUIRED");
      }

      const guild = await client.guilds.fetch(guildId);
      const planned = await inspectServerDiff(
        guild,
        builderConfig.template || fallbackTemplate
      );
