-- CRAZZY PROJECT • 2026-09-23 commercial + auth + operations hardening
-- Idempotent schema synchronization for partner attribution, ghost stock,
-- Discord guild gate support, scalable admin metrics and fulfillment/luck hardening.

begin;

alter table public.reseller_purchases
...
alter table private.product_plan_operations
  drop constraint if exists product_plan_operations_delivery_mode_check;
alter table private.product_plan_operations
  add constraint product_plan_operations_delivery_mode_check
  check (delivery_mode = any(array[
    'internal_stock'::text,
    'ghost_stock'::text,
    'purincash_supplier'::text,
    'lzt_account'::text,
    'manual'::text,
    'service'::text
  ]));
...
create table if not exists public.partners (
  ...
  commission_percent numeric(5,2) not null default 15 check (commission_percent >= 0 and commission_percent <= 50),
...
values('Discord Invite URL','DISCORD_INVITE_URL','','Convite público usado pelo gate de acesso da CRAZZY PROJECT.','')
...
revoke all on table public.partner_plan_rules from anon,authenticated;
...
revoke all on table public.academy_tutorial_plans from anon,authenticated;

grant select on table public.partners to authenticated;
grant select on table public.partner_plan_rules to authenticated;
...
grant select on table public.partner_attributions to authenticated;
grant select,insert,update,delete on table public.academy_tutorial_plans to authenticated;

grant all on table public.partners to service_role;
grant all on table public.partner_plan_rules to service_role;
...
grant all on table public.academy_tutorial_plans to service_role;
...
create policy "Admins manage partner plan rules" on public.partner_plan_rules for all to authenticated
using (private.has_role((select auth.uid()),'admin'::public.app_role))
with check (private.has_role((select auth.uid()),'admin'::public.app_role));
...
create policy "Admins manage partner payouts" on public.partner_payouts for all to authenticated
using (private.has_role((select auth.uid()),'admin'::public.app_role))
with check (private.has_role((select auth.uid()),'admin'::public.app_role));
...
create policy "Admins manage partner commissions" on public.partner_commissions for all to authenticated
using (private.has_role((select auth.uid()),'admin'::public.app_role))
with check (private.has_role((select auth.uid()),'admin'::public.app_role));
...
drop policy if exists "Admins manage academy tutorial plans" on public.academy_tutorial_plans;
create policy "Admins manage academy tutorial plans" on public.academy_tutorial_plans for all to authenticated
using (private.has_role((select auth.uid()),'admin'::public.app_role))
with check (private.has_role((select auth.uid()),'admin'::public.app_role));
...
CREATE OR REPLACE FUNCTION private.sync_reseller_purchase_total()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO 'public', 'private', 'pg_temp'
AS $function$
begin
  if tg_op='INSERT' then
    ...
    update public.resellers
    set total_purchases=greatest(0,total_purchases-1), updated_at=now()
...
begin
  ...
  select array_agg(x.id order by x.created_at),coalesce(sum(x.commission_amount_cents),0)::integer
  into v_ids,v_amount
  from (
...
drop trigger if exists reseller_purchases_sync_total on public.reseller_purchases;
create trigger reseller_purchases_sync_total
after insert or delete on public.reseller_purchases
for each row execute function private.sync_reseller_purchase_total();

drop trigger if exists payments_capture_partner on public.payments;
create trigger payments_capture_partner
...
create trigger payments_partner_commission
...
create trigger partner_commission_refund_reverse
...
create trigger partner_commission_dispute_reverse
...
set total_purchases=(select count(*)::integer from public.reseller_purchases rp where rp.reseller_id=r.id);
