...
export async function supportAuthHeader() {
  const token = await getVerifiedAccessToken();
  return token ? "Bearer " + token : "";
}

export async function proxySupportJson(response: Response) {
  const body = await response
    .json()
    .catch(() => ({ error: "Resposta inválida do Support." }));
