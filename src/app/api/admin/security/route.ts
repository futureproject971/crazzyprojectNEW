...
export async function GET(){
  ...
  const [events,alerts,credentials]=await Promise.all([
  ...
  if(events.error||alerts.error||credentials.error)return NextResponse.json({error:"SECURITY_MANAGER_UNAVAILABLE"},{status:500});
