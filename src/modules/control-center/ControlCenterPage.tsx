...
import { ADMIN_GROUPS } from "@/core/app-shell/admin-navigation";

export function ControlCenterPage() {
  const [toolQuery, setToolQuery] = useState("");
  const toolGroups = ADMIN_GROUPS.map(group => ({...group, tools: group.tools.filter(tool => (group.title + " " + tool.label + " " + tool.description).toLocaleLowerCase("pt-BR").includes(toolQuery.toLocaleLowerCase("pt-BR").trim()))})).filter(group => group.tools.length);
    ...
    <main className="crz-control-page">
      <div className="crz-container">
        ...
        <section className="crz-admin-tools" aria-labelledby="admin-tools-title">
          ...
          <label className="crz-admin-tool-search">Buscar ferramenta<input className="crz-input" type="search" placeholder="Produto, estoque, cliente, desconto…" value={toolQuery} onChange={event => setToolQuery(event.target.value)} /></label>
          {!toolGroups.length && <p>Nenhuma ferramenta encontrada.</p>}
          <div className="crz-admin-groups">
            {toolGroups.map(group => (
              <details className="crz-admin-group" key={group.title + Boolean(toolQuery)} open={Boolean(toolQuery) || group.title === "Produtos"} id={group.title==="Sistema"?"sistema":undefined}>
