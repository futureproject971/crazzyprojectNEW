 ...
 return <main className="crz-luckmanager"><div className="crz-container">
  ...
  <Dialog open={Boolean(prizeForm)} title={prizeForm?.id?"Editar prêmio":"Novo prêmio"} onClose={()=>!busy&&setPrizeForm(null)}>
   {prizeForm&&<form className="crz-admin-form" onSubmit={e=>{e.preventDefault();void savePrize()}}>
    ...
    {prizeForm.prizeType==="product"&&<><label><span>Produto</span><small>Escolha o produto que será liberado.</small><select className="crz-select" value={prizeForm.productId} onChange={e=>setPrizeForm({...prizeForm,productId:e.target.value,productPlanId:""})}><option value="">Escolha...</option>{products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label><span>Plano</span><small>Opcional.</small><select className="crz-select" value={prizeForm.productPlanId} onChange={e=>setPrizeForm({...prizeForm,productPlanId:e.target.value})}><option value="">Qualquer plano</option>{(selectedProduct?.plans||[]).map((p:Row)=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label><span>Duração em minutos</span><small>Use 0 para não expirar.</small><input className="crz-input" value={prizeForm.durationMinutes} onChange={e=>setPrizeForm({...prizeForm,durationMinutes:e.target.value})}/></label></>}
