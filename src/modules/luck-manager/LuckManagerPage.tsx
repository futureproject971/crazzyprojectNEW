...
const emptyPrize=():PrizeForm=>({label:"",prizeType:"product",weight:"10",stock:"",bonusValue:"0",discountType:"percentage",discountValue:"10",minOrderValue:"0",expiresHours:"72",productId:"",productPlanId:"",durationMinutes:"0",note:"",sortOrder:0});
...
export function LuckManagerPage(){
 ...
 const openPrize=(row?:Row)=>{
  ...
  setPrizeForm({
   ...
   id:row?.id,
   label:row?.label||"",
   prizeType:row?.prize_type||(campaign?.mode==="drop"?"coupon":"product"),
 ...
 return <main className="crz-luckmanager"><div className="crz-container">
  ...
  <Dialog open={Boolean(prizeForm)} title={prizeForm?.id?"Editar prêmio":"Novo prêmio"} onClose={()=>!busy&&setPrizeForm(null)}>
   {prizeForm&&<form className="crz-admin-form" onSubmit={e=>{e.preventDefault();void savePrize()}}>
    ...
    {prizeForm.prizeType==="product"&&<><label><span>Produto</span><small>Escolha o produto que será liberado.</small><select className="crz-select" value={prizeForm.productId} onChange={e=>setPrizeForm({...prizeForm,productId:e.target.value,productPlanId:""})}><option value="">Escolha...</option>{products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label><span>Plano</span><small>Obrigatório. A key e a duração vêm deste plano.</small><select className="crz-select" value={prizeForm.productPlanId} onChange={e=>setPrizeForm({...prizeForm,productPlanId:e.target.value})}><option value="">Escolha o plano</option>{(selectedProduct?.plans||[]).map((p:Row)=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label></>}
