...
export default function Editor(){
  ...
  return <section className="crz-mts-native studio-page wrap">
    <div className="studio-head"><div><span className="eyebrow">MTSOUND&apos;S STUDIO</span><h1>Faça o som <span>pesar.</span></h1><p>FX em tempo real, corte visual e exportação com intro oficial travada.</p></div><label className="upload"><Upload size={17}/> Carregar música<input type="file" accept="audio/*" onChange={e=>choose(e.target.files?.[0])}/></label></div>
    <div className="intro-lock"><div className="lock-icon"><LockKeyhole/></div><div><b>Intro oficial MTSound&apos;s</b><span>Obrigatória em todas as exportações • 2.09s • não pode ser removida</span></div><button onClick={()=>{if(introAudio.current){introAudio.current.currentTime=0;introAudio.current.play();}}}><Headphones size={15}/> ouvir</button></div>
    ...
    <div className="editor-layout">
      <div className="editor-main">
        ...
        <section className="glass fx-section"><div className="panel-title"><div><Sparkles/><span><b>Presets</b><small>um clique e já bate diferente</small></span></div></div><div className="preset-row"><button onClick={()=>preset("original")}>Original</button><button onClick={()=>preset("bass")}>Bass Boost</button><button onClick={()=>preset("blown")}>💥 Grave Estourado</button><button onClick={()=>preset("hall")}>Hall / Reverb</button></div></section>

        <section className="glass fx-section"><div className="panel-title"><div><Gauge/><span><b>Aplique efeitos</b><small>processamento em tempo real</small></span></div><button className="reset" onClick={()=>preset("original")}><RotateCcw size={15}/> resetar</button></div><div className="controls-grid">
        ...
        <section className="glass fx-section"><div className="panel-title"><div><Waves/><span><b>Fade in / Fade out</b><small>entrada e saída suaves</small></span></div></div><div className="fade-row"><Control label="Fade In" hint="segundos" value={fadeIn} suffix="s" min={0} max={10} step={.1} set={setFadeIn}/><Control label="Fade Out" hint="segundos" value={fadeOut} suffix="s" min={0} max={10} step={.1} set={setFadeOut}/></div></section>

        <section className="glass fx-section"><div className="panel-title"><div><Scissors/><span><b>Ferramenta de corte</b><small>a intro não entra neste corte</small></span></div></div><div className="trim-grid"><div><label>Início <b>{fmt(start)}</b></label><input aria-label="Início do corte" type="range" min="0" max={Math.max(.1,duration)} step=".01" value={start} onChange={e=>setStart(Math.min(+e.target.value,end-.05))}/></div><div><label>Fim <b>{fmt(end||duration)}</b></label><input aria-label="Fim do corte" type="range" min="0" max={Math.max(.1,duration)} step=".01" value={end} onChange={e=>setEnd(Math.max(+e.target.value,start+.05))}/></div></div></section>
      </div>

      <aside className="glass export-card"><div className="editor-vinyl"><ReactiveVinyl/></div><div className="export-orb"><Volume2/></div><span className="eyebrow">EXPORT</span><h3>Pronto pra tocar no servidor?</h3><p>A intro oficial entra primeiro. Depois vem apenas o trecho editado com seus efeitos.</p><div className="export-flow"><span>INTRO 🔒</span><i>→</i><span>FX</span><i>→</i><span>WAV</span></div>
        {busy&&<div className="export-progress"><div><span>Processando áudio</span><b>{exportProgress}%</b></div><progress max="100" value={exportProgress}/><small>Você pode continuar vendo a página enquanto o arquivo é preparado.</small></div>}
