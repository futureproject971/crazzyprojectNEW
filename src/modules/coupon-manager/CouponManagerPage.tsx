...
export function CouponManagerPage(){
 ...
 const save=async(row?:Row)=>{
   ...
   const userIds=(row?.users||[]).map((u:Row)=>u.user_id);
   const r=await fetch("/api/admin/coupons",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"save_coupon",id:row?.id,code,discountType:type,discountValue:value,minOrderValue:min,maxUses,active:row?.active!==false,expiresAt:row?.expires_at,origin,productIds,userIds})});
 ...
 return <main className="crz-couponmanager"><div className="crz-container">
   ...
   {selected.users.length>0&&<section><strong>Usuários liberados</strong>{selected.users.slice(0,30).map((u:Row)=><a key={u.user_id} href={"/admin/clientes?userId="+u.user_id}>{u.profile?.username||u.user_id.slice(0,8)}</a>)}</section>}
   </>}</aside></div>
 </div></main>
}
